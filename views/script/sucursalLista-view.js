const SucursalesModals = (function() {
    ...
    const API_URL = getBaseURL() + 'ajax/sucursalAjax.php';
    let currentFilters = {
        busqueda: '',
        estado: ''
    };
    ...
    function bindEvents() {
        ...
        if (busquedaInput) {
            // Actualiza el filtro en tiempo real pero NO recarga (búsqueda manual)
            busquedaInput.addEventListener('input', function() {
                currentFilters.busqueda = this.value.trim();
            });

            // Ejecuta búsqueda manual al presionar Enter
        ...
        const btnSearch = document.querySelector('#filtroSucursales .btn-search');
        if (btnSearch) {
        ...
        // Prevenir submit del formulario (evita recarga al presionar Enter)
        const filtroForm = document.getElementById('filtroSucursales');
        if (filtroForm) {
            ...
            return `
                <div class="sucursal-card ${cardClass}">
                    ...
                    <div class="sucursal-actions">
                        ...
                        onclick="event.stopPropagation(); window.location.href='${getBaseURL()}inventarioLista/?sucursal=${suc.su_id}'">
    ...
    function renderizarCajas(cajas) {
        ...
        tbody.innerHTML = cajas.map((caja, index) => {
            ...
            return `
                <tr class="tr-click" onclick="window.location.href='${getBaseURL()}cajaHistorialLista/?select2=${caja.us_id}'">
                    <td>
    ...
    function exportarPDF() {
        const url = getBaseURL() + 'ajax/sucursalAjax.php?sucursalAjax=exportar_pdf';
